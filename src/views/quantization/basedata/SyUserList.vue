    ...
    <div class="app-container">
        <div class="filter-container">
            <el-form :inline="true" label-width="110px" label-position="right">
                                        <el-row>
                        <el-form-item label=":">
                            <el-input v-model="listQuery.userId" placeholder="" style="width: 300px;" class="filter-item" @keyup.enter.native="handleFilter" />
                        </el-form-item>
                        <el-form-item label=":">
                            <el-input v-model="listQuery.userName" placeholder="" style="width: 300px;" class="filter-item" @keyup.enter.native="handleFilter" />
                        </el-form-item>
                        <el-form-item label=":">
                            <el-date-picker v-model="listQuery.bDayStart" type="date" format="yyyy-MM-dd" value-format="yyyy-MM-dd" style="width: 150px;" placeholder="选择时间" /> -
                            <el-date-picker v-model="listQuery.bDayEnd" type="date" format="yyyy-MM-dd" value-format="yyyy-MM-dd" style="width: 150px;" placeholder="选择时间" />
                        </el-form-item>
                        </el-row>
                ...
                {{ empty_tip }}
                    ...
                    <el-table-column align="center" label="" width="110">
                        <template slot-scope="scope">
                            ...
                            <span>{{ parseTime(new Date(scope.row.bDay) ,'{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                        </template>
                    </el-table-column>
        ...
        <el-dialog title="用户信息编辑" :visible.sync="dialogFormVisible" :close-on-click-modal="false" top="50px">
            <el-form ref="dataForm" :model="temp" label-position="right" label-width="110px" style="width: 400px; margin-left:50px;">
                        <el-form-item label="" prop="type">
                            <el-input v-model="temp.userId" readonly disabled placeholder=""/>
                        </el-form-item>
                        <el-form-item label="" prop="type">
                            <el-input v-model="temp.userName"  placeholder="" />
                        </el-form-item>
                        <el-form-item label="" prop="type">
                            <el-date-picker v-model="temp.bDay" type="datetime" format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择时间" />
                        </el-form-item>
...
export default {
  ...
  methods: {
      ...
      updateData() {
          this.temp.bDay = new Date(this.temp.bDay)
      ...
      editDataDialog(row) {
          ...
          getSyUserDetails(row).then(response => {
              ...
              this.temp.bDay = parseTime(new Date(row.bDay), '{y}-{m}-{d} {h}:{i}:{s}')
