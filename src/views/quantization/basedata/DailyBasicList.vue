<template>
    <div class="app-container">
        ...
        <el-dialog title="个股每日指标编辑" :visible.sync="dialogFormVisible" :close-on-click-modal="false" top="50px">
...
import { getDailyBasicPageList, deleteDailyBasicById, insertDailyBasic, updateDailyBasic, getDailyBasicDetails } from '@/api/quantization/DailyBasicApi'
...
export default {
  ...
  methods: {
      getPageList() { // 查询列表分页
          getDailyBasicPageList(this.listQuery).then(response => {
      ...
      deleteData(row) {
          this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
              ...
              deleteDailyBasicById(row).then(response => {
      ...
      updateData() {
          updateDailyBasic(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      addData() {
          insertDailyBasic(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      editDataDialog(row) {
          ...
          getDailyBasicDetails(row).then(response => {
