    ...
    <div class="app-container">
        <div class="filter-container">
            <el-form :inline="true" label-width="110px" label-position="right">
                        ...
                        <el-form-item label="来源 ts:tushare,ths:同花顺 90：东方财富:">
        ...
        <el-dialog title="概念分类编辑" :visible.sync="dialogFormVisible" :close-on-click-modal="false" top="50px">
            <el-form ref="dataForm" :model="temp" label-position="right" label-width="110px" style="width: 400px; margin-left:50px;">
                        ...
                        <el-form-item label="来源 ts:tushare,ths:同花顺 90：东方财富" prop="type">
                            <el-input v-model="temp.src"  placeholder="来源 ts:tushare,ths:同花顺 90：东方财富" />
                        </el-form-item>
                        <el-form-item label="交易所" prop="type">
                            <el-input v-model="temp.exchange"  placeholder="交易所" />
                        </el-form-item>
                        ...
                        <el-form-item label="N概念指数S特色指数" prop="type">
                            <el-input v-model="temp.type"  placeholder="N概念指数S特色指数" />
                        </el-form-item>
...
import { getConceptPageList, deleteConceptById, insertConcept, updateConcept, getConceptDetails } from '@/api/quantization/ConceptApi'
...
export default {
  ...
  methods: {
      getPageList() { // 查询列表分页
          getConceptPageList(this.listQuery).then(response => {
      ...
      deleteData(row) {
          this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
              ...
              deleteConceptById(row).then(response => {
      ...
      updateData() {
          updateConcept(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      addData() {
          insertConcept(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      editDataDialog(row) {
          ...
          getConceptDetails(row).then(response => {
