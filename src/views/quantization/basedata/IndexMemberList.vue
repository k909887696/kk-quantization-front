...
import { getIndexMemberPageList, deleteIndexMemberById, insertIndexMember, updateIndexMember, getIndexMemberDetails } from '@/api/quantization/IndexMemberApi'
...
export default {
  ...
  methods: {
      getPageList() { // 查询列表分页
          getIndexMemberPageList(this.listQuery).then(response => {
      ...
      deleteData(row) {
          this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
              ...
              deleteIndexMemberById(row).then(response => {
      ...
      updateData() {
          updateIndexMember(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      addData() {
          insertIndexMember(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      editDataDialog(row) {
          ...
          getIndexMemberDetails(row).then(response => {
