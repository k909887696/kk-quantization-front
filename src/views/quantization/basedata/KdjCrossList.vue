<template>
    <div class="app-container">
        ...
        <el-dialog title="kdj交叉点编辑" :visible.sync="dialogFormVisible" :close-on-click-modal="false" top="50px">
...
import { getKdjCrossPageList, deleteKdjCrossById, insertKdjCross, updateKdjCross, getKdjCrossDetails } from '@/api/quantization/KdjCrossApi'
...
export default {
  ...
  methods: {
      getPageList() { // 查询列表分页
          getKdjCrossPageList(this.listQuery).then(response => {
      ...
      deleteData(row) {
          this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
              ...
              deleteKdjCrossById(row).then(response => {
      ...
      updateData() {
          updateKdjCross(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      addData() {
          insertKdjCross(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      editDataDialog(row) {
          ...
          getKdjCrossDetails(row).then(response => {
