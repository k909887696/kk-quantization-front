...
import { getIndexClassifyPageList, deleteIndexClassifyById, insertIndexClassify, updateIndexClassify, getIndexClassifyDetails } from '@/api/quantization/IndexClassifyApi'
...
export default {
  ...
  methods: {
      getPageList() { // 查询列表分页
          getIndexClassifyPageList(this.listQuery).then(response => {
      ...
      deleteData(row) {
          this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
              ...
              deleteIndexClassifyById(row).then(response => {
      ...
      updateData() {
          updateIndexClassify(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      addData() {
          insertIndexClassify(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      editDataDialog(row) {
          ...
          getIndexClassifyDetails(row).then(response => {
