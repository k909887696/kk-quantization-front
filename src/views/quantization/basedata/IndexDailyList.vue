...
import { getIndexDailyPageList, deleteIndexDailyById, insertIndexDaily, updateIndexDaily, getIndexDailyDetails } from '@/api/quantization/IndexDailyApi'
...
export default {
  ...
  methods: {
      getPageList() { // 查询列表分页
          getIndexDailyPageList(this.listQuery).then(response => {
      ...
      deleteData(row) {
          this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
              ...
              deleteIndexDailyById(row).then(response => {
      ...
      updateData() {
          updateIndexDaily(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      addData() {
          insertIndexDaily(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      editDataDialog(row) {
          ...
          getIndexDailyDetails(row).then(response => {
