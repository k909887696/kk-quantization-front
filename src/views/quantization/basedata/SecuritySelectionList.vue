    ...
    <div class="app-container">
        <div class="filter-container">
            <el-form :inline="true" label-width="110px" label-position="right">
                        ...
                        <el-form-item label=":">
                            <el-input v-model="listQuery.selectType" placeholder="" style="width: 300px;" class="filter-item" @keyup.enter.native="handleFilter" />
                        </el-form-item>
                        </el-row>
        ...
        <el-dialog title="个人自选股编辑" :visible.sync="dialogFormVisible" :close-on-click-modal="false" top="50px">
            <el-form ref="dataForm" :model="temp" label-position="right" label-width="110px" style="width: 400px; margin-left:50px;">
                        ...
                        <el-form-item label="" prop="type">
                            <el-input v-model="temp.selectType"  placeholder="" />
                        </el-form-item>
...
import { getSecuritySelectionPageList, deleteSecuritySelectionById, insertSecuritySelection, updateSecuritySelection, getSecuritySelectionDetails } from '@/api/quantization/SecuritySelectionApi'
...
export default {
  ...
  methods: {
      getPageList() { // 查询列表分页
          getSecuritySelectionPageList(this.listQuery).then(response => {
      ...
      deleteData(row) {
          this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
              ...
              deleteSecuritySelectionById(row).then(response => {
      ...
      updateData() {
          updateSecuritySelection(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      addData() {
          insertSecuritySelection(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      editDataDialog(row) {
          ...
          getSecuritySelectionDetails(row).then(response => {
