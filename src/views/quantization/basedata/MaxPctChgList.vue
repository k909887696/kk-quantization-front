    ...
    <div class="app-container">
        <div class="filter-container">
            <el-form :inline="true" label-width="110px" label-position="right">
                        ...
                        <el-form-item label=":">
                            <el-input v-model="listQuery.maxPctChg" placeholder="" style="width: 300px;" class="filter-item" @keyup.enter.native="handleFilter" />
                        </el-form-item>
                        </el-row>
        ...
        <el-dialog title="各个市场涨跌幅限制编辑" :visible.sync="dialogFormVisible" :close-on-click-modal="false" top="50px">
            <el-form ref="dataForm" :model="temp" label-position="right" label-width="110px" style="width: 400px; margin-left:50px;">
                        ...
                        <el-form-item label="" prop="type">
                            <el-input v-model="temp.maxPctChg"  placeholder="" />
                        </el-form-item>
...
import { getMaxPctChgPageList, deleteMaxPctChgById, insertMaxPctChg, updateMaxPctChg, getMaxPctChgDetails } from '@/api/quantization/MaxPctChgApi'
...
export default {
  ...
  methods: {
      getPageList() { // 查询列表分页
          getMaxPctChgPageList(this.listQuery).then(response => {
      ...
      deleteData(row) {
          this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
              ...
              deleteMaxPctChgById(row).then(response => {
      ...
      updateData() {
          updateMaxPctChg(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      addData() {
          insertMaxPctChg(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      editDataDialog(row) {
          ...
          getMaxPctChgDetails(row).then(response => {
