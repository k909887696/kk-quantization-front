<template>
    <div class="app-container">
        <div class="filter-container">
            <el-form :inline="true" label-width="110px" label-position="right">
                        ...
                        <el-form-item label="前收盘价:">
                            <el-input v-model="listQuery.preClose" placeholder="前收盘价" style="width: 300px;" class="filter-item" @keyup.enter.native="handleFilter" />
                        </el-form-item>
                        ...
                        <el-form-item label="成交量:">
                            <el-input v-model="listQuery.vol" placeholder="成交量" style="width: 300px;" class="filter-item" @keyup.enter.native="handleFilter" />
                        </el-form-item>
                        <el-form-item label="成交额:">
                            <el-input v-model="listQuery.amount" placeholder="成交额" style="width: 300px;" class="filter-item" @keyup.enter.native="handleFilter" />
                        </el-form-item>
                        </el-row>
        ...
        >
            <template slot="empty">
                {{ empty_tip }}
                    ...
                    <el-table-column align="center" label="前收盘价" width="150">
                        <template slot-scope="scope">
                            {{ scope.row.preClose }}
                        </template>
                    </el-table-column>
        ...
        <el-dialog title="概念日线行情编辑" :visible.sync="dialogFormVisible" :close-on-click-modal="false" top="50px">
            <el-form ref="dataForm" :model="temp" label-position="right" label-width="110px" style="width: 400px; margin-left:50px;">
                        ...
                        <el-form-item label="前收盘价" prop="type">
                            <el-input v-model="temp.preClose"  placeholder="前收盘价" />
                        </el-form-item>
                        ...
                        <el-form-item label="成交量" prop="type">
                            <el-input v-model="temp.vol"  placeholder="成交量" />
                        </el-form-item>
                        <el-form-item label="成交额" prop="type">
                            <el-input v-model="temp.amount"  placeholder="成交额" />
                        </el-form-item>
...
import { getConceptDailyPageList, deleteConceptDailyById, insertConceptDaily, updateConceptDaily, getConceptDailyDetails } from '@/api/quantization/ConceptDailyApi'
...
export default {
  ...
  methods: {
      getPageList() { // 查询列表分页
          getConceptDailyPageList(this.listQuery).then(response => {
      ...
      deleteData(row) {
          this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
              ...
              deleteConceptDailyById(row).then(response => {
      ...
      updateData() {
          updateConceptDaily(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      addData() {
          insertConceptDaily(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      editDataDialog(row) {
          ...
          getConceptDailyDetails(row).then(response => {
