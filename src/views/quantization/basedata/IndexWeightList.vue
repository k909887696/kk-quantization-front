    ...
    <div class="app-container">
        <div class="filter-container">
            <el-form :inline="true" label-width="110px" label-position="right">
                        ...
                        <el-form-item label="成分代码:">
                            <el-input v-model="listQuery.conCode" placeholder="成分代码" style="width: 300px;" class="filter-item" @keyup.enter.native="handleFilter" />
                        </el-form-item>
        ...
        <el-dialog title="指数成分权重编辑" :visible.sync="dialogFormVisible" :close-on-click-modal="false" top="50px">
            <el-form ref="dataForm" :model="temp" label-position="right" label-width="110px" style="width: 400px; margin-left:50px;">
                        ...
                        <el-form-item label="成分代码" prop="type">
                            <el-input v-model="temp.conCode" readonly disabled placeholder="成分代码"/>
                        </el-form-item>
...
import { getIndexWeightPageList, deleteIndexWeightById, insertIndexWeight, updateIndexWeight, getIndexWeightDetails } from '@/api/quantization/IndexWeightApi'
...
export default {
  ...
  methods: {
      getPageList() { // 查询列表分页
          getIndexWeightPageList(this.listQuery).then(response => {
      ...
      deleteData(row) {
          this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
              ...
              deleteIndexWeightById(row).then(response => {
      ...
      updateData() {
          updateIndexWeight(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      addData() {
          insertIndexWeight(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      editDataDialog(row) {
          ...
          getIndexWeightDetails(row).then(response => {
