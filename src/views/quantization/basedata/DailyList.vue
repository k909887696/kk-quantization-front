...
import { getDailyPageList, deleteDailyById, insertDaily, updateDaily, getDailyDetails } from '@/api/quantization/DailyApi'
...
export default {
  ...
  data() {
    const listStart = parseTime(addTime(new Date(), -1, 'month'), '{y}{m}') +'01'
    const listEnd = parseTime(new Date(), '{y}{m}{d}')
    console.log('listStart:', listStart,listEnd)
      return {
          ...
          listQuery: {
      tsCode: undefined,
              tradeDateStart: listStart,
              tradeDateEnd: listEnd,
      open: undefined,
      high: undefined,
      low: undefined,
      close: undefined,
  ...
  methods: {
      getPageList() { // 查询列表分页
          getDailyPageList(this.listQuery).then(response => {
      ...
      deleteData(row) {
          this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
              ...
              deleteDailyById(row).then(response => {
      ...
      updateData() {
          updateDaily(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      addData() {
          insertDaily(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      editDataDialog(row) {
          ...
          getDailyDetails(row).then(response => {
