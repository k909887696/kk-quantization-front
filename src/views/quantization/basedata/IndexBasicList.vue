...
import { getIndexBasicPageList, deleteIndexBasicById, insertIndexBasic, updateIndexBasic, getIndexBasicDetails } from '@/api/quantization/IndexBasicApi'
...
export default {
  ...
  methods: {
      getPageList() { // 查询列表分页
          getIndexBasicPageList(this.listQuery).then(response => {
      ...
      deleteData(row) {
          this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
              ...
              deleteIndexBasicById(row).then(response => {
      ...
      updateData() {
          updateIndexBasic(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      addData() {
          insertIndexBasic(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      editDataDialog(row) {
          ...
          getIndexBasicDetails(row).then(response => {
