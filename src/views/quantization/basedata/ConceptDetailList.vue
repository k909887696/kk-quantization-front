...
import { getConceptDetailPageList, deleteConceptDetailById, insertConceptDetail, updateConceptDetail, getConceptDetailDetails } from '@/api/quantization/ConceptDetailApi'
...
export default {
  ...
  methods: {
      getPageList() { // 查询列表分页
          getConceptDetailPageList(this.listQuery).then(response => {
      ...
      deleteData(row) {
          this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
              ...
              deleteConceptDetailById(row).then(response => {
      ...
      updateData() {
          updateConceptDetail(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      addData() {
          insertConceptDetail(this.temp).then(response => {
              this.dialogFormVisible = false
              this.getPageList()
          })
      },
      editDataDialog(row) {
          ...
          getConceptDetailDetails(row).then(response => {
